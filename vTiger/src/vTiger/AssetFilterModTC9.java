...
public class AssetFilterModTC9 {
	...
	public static void main(String[] args) {
		...
		{
			System.out.println("Filter created with metrics present in the list.");
		}
		else
		{
			System.out.println("Filter created with metrics not present in the list.");
		}
		
		driver.findElement(By.cssSelector("img[src='themes/softed/images/Home.PNG']")).click();
		
		List<WebElement> wbList = driver.findElements(By.xpath("//div[@id='editRowmodrss_5']/following-sibling::table/tbody/tr/td[2]"));
		
		boolean flag=false;
		
		for(WebElement w: wbList)
		{
			System.out.println(w.getText().replace("( Administrator)", ""));
			
			if(w.getText().contains("cust_filter1"))
			{
				flag = true;
				break;
			}
		...
		if(flag)
		{
			System.out.println("cust_filter1 is present in metrics");
		}
		else
		{
			System.out.println("cust_filter1 not present in metrics");
		}
